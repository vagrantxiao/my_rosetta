...
#include "bin_conv_weights.h"
#pragma HLS ARRAY_PARTITION variable=wt_mem_0 complete dim=1
#pragma HLS ARRAY_PARTITION variable=wt_mem_1 complete dim=1
#pragma HLS ARRAY_PARTITION variable=wt_mem_2 complete dim=1
#pragma HLS ARRAY_PARTITION variable=wt_mem_3 complete dim=1
#pragma HLS ARRAY_PARTITION variable=wt_mem_4 complete dim=1

  NormComp nc;
  switch(layer)
  {
    case 0:
      load_kh(nc, kh_mem_0, kh_index);
      break;
    case 1:
      load_kh(nc, kh_mem_1, kh_index);
      break;
    case 2:
      load_kh(nc, kh_mem_2, kh_index);
      break;
    case 3:
      load_kh(nc, kh_mem_3, kh_index);
      break;
    default:
      load_kh(nc, kh_mem_4, kh_index);
  }
...
#pragma HLS ARRAY_PARTITION variable=wt_word_buffer complete dim=0
        ...
        for (IdxType m = 0; m < CONVOLVERS; ++m) {
          ...
          */
          ...
          {
            case 0:
              wt_wrd = wt_mem_0[m][wt_addr];
              break;
            case 1:
              wt_wrd = wt_mem_1[m][wt_addr];
              break;
            case 2:
              wt_wrd = wt_mem_2[m][wt_addr];
              break;
            case 3:
              wt_wrd = wt_mem_3[m][wt_addr];
              break;
            default:
              wt_wrd = wt_mem_4[m][wt_addr];
          }
          wt_word_buffer[m] = wt_wrd >> ap_uint<6>(WT_SIZE*wt_offset);
        }
...
#pragma HLS PIPELINE
      ...
      for (IdxType j = 0; j < CONVOLVERS; ++j) {
        ...
        {
          case 0:
            wt_wrd = wt_mem_0[j][wt_addr / CONVOLVERS];
            break;
          case 1:
            wt_wrd = wt_mem_1[j][wt_addr / CONVOLVERS];
            break;
          default:
            wt_wrd = wt_mem_2[j][wt_addr / CONVOLVERS];
        }

        ap_int<65> x = wt_wrd ^ in_wrd;
    ...
    if (layer_type == LAYER_DENSE)
      kh_addr = o / KH_PER_WORD;
    else
      kh_addr = o / (const unsigned)2;
    Word kh_word;
    switch(layer)
    {
      case 0:
...
unsigned char top(
  ...
  ap_uint<22> n_inputs_list[9]=				{3,   128, 128, 256, 256, 512, 8192, 1024, 1024};
  ap_uint<22> n_outputs_list[9]=			{128, 128, 256, 256, 512, 512, 1024, 1024, 10};
  ap_uint<22> input_words_list[9]=			{1024, 0,  0,   0,   0,   0,   0,    0,    0};
  ap_uint<22> output_words_list[9]=			{0,    0,  0,   0,   0,   0,   0,    0,    1};
  ap_uint<3> layer_mode_list[9]=			{1,    3,  3,   3,   3,   3,   5,    5,    7};
  ap_uint<1> dmem_mode_list[9]=				{1,    0,  1,   0,   1,   0,   1,    0,    1};
  ap_uint<2> width_mode_list[9]=			{2,    2,  1,   1,   0,   0,   0,    0,    0};
  ap_uint<2> norm_mode_list[9]=				{1,    2,  1,   2,   1,   2,   1,    1,    1};
  ap_uint<3> layer_list[9]=					{0,    1,  2,   3,   4,   5,   6,    7,    0};

  Word dmem_o[128]={0};


for(int TopLoopNum=0; TopLoopNum<9; TopLoopNum++){
  Address    n_inputs=n_inputs_list[TopLoopNum];
  Address    n_outputs=n_outputs_list[TopLoopNum];
  Address    input_words=input_words_list[TopLoopNum];
  Address    output_words=output_words_list[TopLoopNum];
  ap_uint<3> layer_mode=layer_mode_list[TopLoopNum];  // [0]='new layer', [2:1]='conv1,conv,dense,last'
  ap_uint<1> dmem_mode=dmem_mode_list[TopLoopNum]; // 0 means dmem[0] is input
  ap_uint<2> width_mode=width_mode_list[TopLoopNum];  // 0=8'b, 1=16'b, 2=32'b
  ap_uint<2> norm_mode=norm_mode_list[TopLoopNum];   // 0='do nothing', 1='do norm', 2='do pool'
  ap_uint<3> layer=layer_list[TopLoopNum];
...
#pragma HLS PIPELINE
    if (layer_type == LAYER_CONV) {
      ...
      dmem[d_i_idx][bank_idx][(bank_off<<(2*width_mode)) + img_off] = Input_1[input_words];
    }
    else if (layer_type == LAYER_CONV1){
      dmem[d_i_idx][i/C_DMEM_WORDS][i%C_DMEM_WORDS] =  Input_1[input_words];
    }else{

      dmem[d_i_idx][i%CONVOLVERS][i/CONVOLVERS] =  Input_1[input_words];
    }
...
#pragma HLS PIPELINE
	...
	unsigned char final_out = (unsigned char)dmem_o[0](7, 0);
	return final_out;
	//for(int i=0; i<4; i++){
	//	Output_1.write(16);
	//	Output_1.write(16);
	//	Output_1.write(dmem_o[i](63,32));
	//	Output_1.write(dmem_o[i](31,0));
	//}
}
